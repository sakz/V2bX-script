...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/sakz/V2bX-script/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/sakz/V2bX-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
install_bbr() {
    bash <(curl -L -s https://raw.githubusercontent.com/sakz/Linux-NetSpeed-Chikage/master/tcpx.sh)
}

update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/sakz/V2bX-script/master/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
